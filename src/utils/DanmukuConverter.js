...
class DanmukuConverter {
  ...
  convert(xmlString) {
    const xmlJson = JSON.parse(XmlJs.xml2json(xmlString, { compact: true }));
...
Style: Danmaku,${this.config.font},${this.config.font_size},&H${this.config.alpha}FFFFFF,&H${
      this.config.alpha
    ...
    for (let i = 0; i < dElements.length; i++) {
      ...
      let move1 = this.config.PlayResX + (text.length * this.config.font_size) / 2;
      let move24 = this.config.font_size;
      const move3 = 0 - (text.length * this.config.font_size) / 2;
  ...
  getDanmukuFitLine({ danmukuPosition, danmukuText }, lineRecords, type) {
    ...
    if (type <= 3) {
      for (let i = 0; i < lineRecords[0].length; i++) {
        ...
        let danmakuLength = lineRecords[0][i].danmukuText.length * this.config.font_size;
        ...
        // 当前弹幕最后一刻完全显示在屏幕的时间
        danmakuLength = danmukuText.length * this.config.font_size;
